...
三个第三方项目以固定快照内置，但不由本仓库维护或担保。上游指令文件可按路由读取；任何脚本、hooks、MCP、安装器、浏览器或外部服务在使用前仍须检查代码、依赖、权限、网络访问和输出范围。发现内置快照供应链问题时，请同时提供 `modules/LOCK.json` 中的来源与提交以及受影响文件的清单哈希。
